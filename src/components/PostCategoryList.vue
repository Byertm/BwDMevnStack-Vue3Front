...
<script setup lang="ts">
	...
	const { getCategoryPosts, isCategoryPosts } = storeToRefs(postStore);
	...
	const setPostsData = () => {
		//Todo: Tüm article convert işlemleri değişmeli
		articles.value = getCategoryPosts.value?.map((post: IPost) => formatPost(post)) || [];
		// articles.value = getCategoryPosts.value;
	};

	watch([() => getCategoryPosts.value, () => isCategoryPosts.value], () => setPostsData());

	onMounted(async () => {
		if (isCategoryPosts.value) {
			await postStore.getAllByCategory(<string>route?.params?.slug);
